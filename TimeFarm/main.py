...
from utils.TimeFarm import TimeFarm
        ...
        if USE_PROXY:
            ...
            for thread, account in enumerate(accounts):
                if account in proxy_dict:
                    tasks.append(
                        asyncio.create_task(TimeFarm(account=account, thread=thread, proxy=proxy_dict[account]).main()))
                else:
                    tasks.append(asyncio.create_task(TimeFarm(account=account, thread=thread).main()))
        else:
            for thread, account in enumerate(accounts):
                tasks.append(asyncio.create_task(TimeFarm(account=account, thread=thread).main()))
